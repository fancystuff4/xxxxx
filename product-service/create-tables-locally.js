...
async function getDynamoDBTableResources() {
  ...
  const productTable = yaml.loadAll(fs.readFileSync(SERVERLESS_CONFIG), {
    schema: cloudformationSchema,
  })[0].resources.Resources;
  console.log(
    yaml.loadAll(productTable.productTable, { schema: cloudformationSchema }),
  );
...
(async function main() {
  ...
  for await ([logicalId, definition] of tables) {
    const {
      Properties: { BillingMode, TableName, AttributeDefinitions, KeySchema },
    } = definition;
