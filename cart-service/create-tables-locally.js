...
async function getDynamoDBTableResources() {
  ...
  const cartTable = yaml.loadAll(fs.readFileSync(SERVERLESS_CONFIG), {
    schema: cloudformationSchema,
  })[0].resources.Resources;
  console.log(
    yaml.loadAll(cartTable.cartTable, { schema: cloudformationSchema }),
  );
...
(async function main() {
  ...
  for await ([logicalId, definition] of tables) {
    const {
      Properties: { BillingMode, TableName, AttributeDefinitions, KeySchema, GlobalSecondaryIndexes },
    } = definition;
    console.info('dddd', logicalId, definition);
    // console.log(definition.Properties.AttributeDefinitions);
    // console.log(definition.Properties.KeySchema);
    // console.log(definition.Properties.GlobalSecondaryIndexes)
    // console.log('GSI KeySchema', definition.Properties.GlobalSecondaryIndexes[0].KeySchema)
    if (
