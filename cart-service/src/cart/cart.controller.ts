...
export class CartController {
    ...
    if (updateCartDto.quantity && typeof updateCartDto.quantity === 'number') {
      try {
        ...
        if (userCart && userCart.id === cartID) {
          const modifiedUserCart = await this.cartService.modifyItemInUserCart(
            userCart.id,
            userCart.items,
            lineItemID,
      ...
      return res.status(HttpStatus.BAD_REQUEST).json({
        statusCode: HttpStatus.BAD_REQUEST,
        message:
          'Update Cart: Please specify the quantity of the item correctly',
      });
    }
  }
  ...
  async removeUserCartItem(
    ...
    try {
      const userCart: any = await this.cartService.findUserCart(userID);
      if (userCart && userCart.id === cartID) {
        const result = await this.cartService.removeItemFromUserCart(
          userCart.id,
          userCart.items,
          lineItemID,
        );
        if (result.ok) {
          ...
          if (!userCart.items.length) {
            // while deleting the last item in the user cart
  ...
  async addToCart(
    ...
    if (!userCart) {
      ...
      const itemExistsInCart = this.cartService.findItemInCart(
        userCart.items,
        body.variant.variantID,
      );
      var updatedCart: any;
      if (itemExistsInCart) {
        updatedCart = await this.cartService.updateItemInCart(
          body,
          userCart.id,
          userCart.items,
        );
        ...
        // when the item doesn't exist in his cart
        updatedCart = await this.cartService.insertIntoCartItems(
          body,
          userCart.id,
        );
        if (updatedCart.ok) {
