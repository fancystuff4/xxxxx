...
export class TenantRepository {
    ...
    try {
      ...
      if (paginateTenantDto.lastItem) {
        params['ExclusiveStartKey'] = {
          PK: paginateTenantDto.lastItem,
          SK: paginateTenantDto.lastItem,
        };
      }
    ...
    try {
      ...
      result = await client
        .update({
          ...
          UpdateExpression:
            'set #fullname = :n, #address = :address,#status=:status',
  ...
  async updateTenantStatus(
    getTenantDto: GetTenantDto,
    updateTenantStatus: UpdateUserStatus,
  ) {
    let result;
    try {
      result = await client
  ...
  async addPaymentCredentials(
    superAdminId,
    tenantId,
    addPaymentCredentialsDto: AddPaymentCredentialsDto,
  ) {
    let result;
    const name = addPaymentCredentialsDto.paymentService;
    const paymentProperties = addPaymentCredentialsDto.paymentProperties;
    const paymentPropertyValues =
      addPaymentCredentialsDto.paymentPropertyValues;
    try {
      ...
      if (Object.keys(response).length != 0) {
        if (
          response?.Item?.paymentdata &&
          response.Item.paymentdata.hasOwnProperty(name)
        ) {
          var keys = Object.keys(response.Item.paymentdata[name]);
          const checkForAll = paymentProperties.every((elem) =>
            keys.includes(elem),
          );
          if (paymentProperties.length == keys.length && checkForAll) {
            ...
            keys.forEach((k, i) => {
              paymentCredentials[
                response.Item.paymentdata[name][paymentProperties[i]]
              ] = paymentPropertyValues[i];
            });
            result = await client
              .update({
                TableName: 'tenant',
            ...
            return {
              ok: false,
              message: 'All Valid Payment Properties Required',
            };
          }
        } else {
  ...
  async addPaymentService(
    superAdminId,
    addPaymentServiceDto: AddPaymentServiceDto,
  ) {
    let result;
    const paymentService = addPaymentServiceDto.paymentProperties.reduce(
      (a, v) => ({
        ...a,
        [v]: uuid(),
      }),
    ...
    try {
      ...
      result = await client
        .update({
          ...
          ConditionExpression: 'attribute_exists(PK) AND attribute_exists(SK)',
          UpdateExpression:
            'set #paymentdata.#cred = if_not_exists(#paymentdata.#cred,:paymentData)',
          ExpressionAttributeValues: {
  ...
  async editPaymentService(
    superAdminId,
    serviceName,
    editPaymentServiceDto: EditPaymentServiceDto,
  ) {
    let result, response;
    ...
    try {
      ...
      if (
        ...
        if (response.Item.paymentdata[name][newPropertyValue]) {
          ...
          return {
            ok: false,
            message: 'Payment Service Property Should Be Unique',
          };
        } else {
          if (response.Item.paymentdata[name][propertyName]) {
            const value = response.Item.paymentdata[name][propertyName];
            params['UpdateExpression'] =
              'set #paymentdata.#cred.#name =:paymentData REMOVE #paymentdata.#cred.#old';
            ...
            const value = uuid();
            params['UpdateExpression'] =
              'set #paymentdata.#cred.#name =:paymentData';
            params['ExpressionAttributeNames'] = {
              '#paymentdata': 'paymentdata',
              '#cred': name,
              '#name': newPropertyValue,
